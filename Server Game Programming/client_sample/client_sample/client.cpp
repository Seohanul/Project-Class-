...
void ProcessPacket(char *ptr)
{
	...
	case SC_KILL_PLAYER:
	{
		sc_packet_kill_player *my_packet = reinterpret_cast<sc_packet_kill_player *>(ptr);
		int other_id = my_packet->id;
		if (other_id == g_myid) {
			avatar.hide();
		}
...
//	for (auto &pl : players) pl.draw();
	...
	text.setFont(g_font);
	char buf[100];
	sprintf_s(buf, "(%d, %d)\n HP : %d\n Level : %d", avatar.m_x, avatar.m_y,avatar.HP, avatar.level);
	text.setString(buf);
	g_window->draw(text);
