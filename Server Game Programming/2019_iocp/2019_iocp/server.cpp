...
void send_hp_change_packet(int id, int target)
{
	sc_packet_hp packet;
	packet.id = id;
	packet.size = sizeof(packet);
...
void send_kill_player_packet(int client, int leaver)
{
	sc_packet_kill_player packet;
	packet.id = leaver;
	packet.size = sizeof(packet);
	packet.type = SC_KILL_PLAYER;
	send_packet(client, &packet);
...
void ProcessMove(int id, short x, short y)
{
...
void ProcessAttack(int id,short x, short y)
{
	for (auto near_id : clients[id]->near_id) {
		if (Is_NPC(near_id)) {
			int num = 0;
			if (1 <= abs(x - clients[near_id]->x)) num++;
			if (1 <= abs(y - clients[near_id]->y)) num++;

	//		cout << "num" << endl;
			if (1 == num) {
				clients[near_id]->HP -= 10;
				cout << clients[near_id]->HP << endl;
			
				send_hp_change_packet(id, near_id);
				
				if (0 == clients[near_id]->HP) {
					clients[id]->exp += 50;
					if (clients[id]->level * 100 < clients[id]->exp) {
						clients[id]->level++;
						send_levelup_packet(id);
					}
		

					send_kill_player_packet(id, near_id);
					clients.unsafe_erase(near_id);
				}
			}
		}
	}
